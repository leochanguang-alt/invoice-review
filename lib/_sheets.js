...
function cleanEnv(v) {
  if (!v) return "";
  v = v.trim();
  // Recursively remove leading/trailing single or double quotes
...
export function getDriveAuth() {
  ...
  // Enhanced private key cleaning logic
  // 1. Handle escaped newlines
  key = key.replace(/\\n/g, "\n");
  // 2. If private key was accidentally included in JSON structure or has extra spaces, extract only PEM part
  ...
  // 3. Remove possible leading/trailing spaces from each line (sometimes copied from web pages)
...
export async function findFirstWaitingRow(sheets, sheetName, statusCol1Based) {
  ...
  for (let i = 0; i < values.length; i++) {
    if (norm(values[i]?.[0]) === WAITING_STATUS) return i + 2; // Starting from row 2
  }
  return null;
}
