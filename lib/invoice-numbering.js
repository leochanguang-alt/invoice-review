...
export function extensionFromOldKey(oldKey) {
    // Only strip a query string (?...). Keep '#' — R2 object keys legitimately contain '#'
    // (e.g. "Receipt from Mouse Tail Coffee #Hfu2.pdf"), and splitting on '#' would drop
    // the extension and wrongly default to '.pdf'.
    const keyWithoutQuery = String(oldKey).split('?', 1)[0];
    const basename = keyWithoutQuery.split("/").pop() || "";
