...
async function handleTransactions(req, body, json, res) {
    ...
    let query = supabase
        .from('bank_transactions')
        .select('*, invoices:matched_invoice_id(id, vendor, amount, currency, invoice_date, generated_invoice_id, status, amount_hkd)')
    ...
    if (unmatched.length) {
        ...
        const { data: invoices } = await supabase
            .from('invoices')
            .select('id, vendor, amount, currency, invoice_date, generated_invoice_id, status, amount_hkd, reconciled_at, bank_transaction_id, deleted_at')
            .is('deleted_at', null)
            .is('reconciled_at', null)
            .is('bank_transaction_id', null)
            .gte('invoice_date', addDays(minDate, -15))
            .lte('invoice_date', addDays(maxDate, 10));
...
async function handleMatch(body, json, res) {
    ...
    const { data: invoice } = await supabase
        .from('invoices')
        .select('id, vendor, amount, currency, invoice_date, generated_invoice_id, status, amount_hkd')
        .eq('id', invoice_id)
        .single();
...
async function handleUnmatch(body, json, res) {
    ...
    if (txErr || !tx) return json(res, 404, { success: false, message: 'Transaction not found' });
    if (!tx.matched_invoice_id) {
        return json(res, 400, { success: false, message: 'Transaction is not matched' });
    }
    ...
    return json(res, 200, { success: true, transaction_id, invoice_id: invoiceId });
}
